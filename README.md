...
## **Горячие клавиши**
```bash
&&        # команды можно указать списком, разделяя их двумя амперсандами
↑ и ↓        # перемещение по нескольким последним командам
Tab        # можно набрать первые символы имени и дважды нажать tab, командная строка допишет путь сама.
ctrl + u        # очистить строку ввода
esc + :qa!        # выйти из редактора vim  
```
...
rm -rf <папка>        # удалить папку и все ее содержимое
rmdir <папка>        # удалить папку. Раотает только с пустыми папками
touch <имя-файла.расширение>        # Создать файл
cp file.txt ~/<папка>        # скопируй файл/папку в подпапку домашней директории
mv file.txt ~/<папка>        # перемести файл/папку в подпапку домашней директории
...
```bash
#Сначала создаем репозиторйи в GitHub
git remote -v        # Просмотр списка удалённых репозиториев  
git remote add origin <url>        # Добавление удалённого репозитория. url = ssh.
